...
afterEach(function () {
    ...
    foreach (['src/Domain', 'src/App', 'src/Foundation', 'src/Infrastructure', 'src/Integrations', 'src/Support'] as $path) {
...
it('adds the domain driven design autoload mappings to composer.json', function () {
    ...
    expect($composer['autoload']['psr-4'])
        ...
        ->toHaveKey('App\\Modules\\', 'src/App/Modules/')
...
it('creates the domain driven design directories and helper file', function () {
    ...
    expect(File::isDirectory(base_path('src/App/Modules')))->toBeTrue();
